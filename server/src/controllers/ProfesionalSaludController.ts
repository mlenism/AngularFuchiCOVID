...
import tipoIDController from './TipoIDController';
...
class  ProfesionalSaludController {
    ...
    public async getProfesionales(req: Request, res: Response): Promise<Response> {
        try {
            const profesionales: QueryResult = await pool.query('SELECT prof.id,'
            +'prof.nombre,'
            +'prof.apellido,'
            +'prof.contrasenia,'
            +'ubic.direccion,'
    ...
    public async setProfesional(req: Request, res: Response): Promise<Response> {
        try {
            ...
            await pool.query('INSERT INTO profesional_salud (id, nombre, apellido, id_tipoid, id_universidad, id_entidad, contrasenia) '
                +'VALUES ($1,$2,$3,$4,$5,$6,$7)',[id, nombre, apellido, tipoID, universidad, entidad, contrasenia]);
            await pool.query('INSERT INTO registro_profesional_salud (id_miembro_secretaria_salud, id_profesional_salud) '
                +'VALUES ($1, $2)', [id_miembro_secretaria, id]);
            await pool.query('INSERT INTO ubicacion_profesional_salud (id_profesional_salud, id_barrio, direccion) '
