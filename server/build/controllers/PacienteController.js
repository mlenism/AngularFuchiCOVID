...
class PacienteController {
    ...
    async updatePacientes(req, res) {
        try {
            const { id_miembro_secretaria, id, nombre, apellido, medico, direccion, barrio, tipoID, integrantes, ciudad, latitud, longitud } = req.body;
            await database_1.pool.query('UPDATE paciente nombre=$1, apellido=$2, id_tipoid=$3, numerodeintegrantes=$4, ciudad_contagio=$5, id_medico=$5 '
                + 'WHERE id=$6', [nombre, apellido, tipoID, integrantes, ciudad, medico, id]);
            await database_1.pool.query('UPDATE ubicacion_paciente id_barrio=$1, direccion=$2, latitud=$3, longitud=$4 WHERE id_paciente=$5', [barrio, direccion, latitud, longitud, id]);
